...
{output_dir}/
└── analyzer/
    ├── knowledge_draft.json    # 结构化知识（脚本产出）
    ├── knowledge_final.json    # 完整知识（AI 增强后）
    └── views/
        ├── mapping.xlsx        # 字段映射
...
能。这个 skill 基于 opencode 标准，把 `.opencode/skills/dws-pipeline-analyzer/` 目录和 `.opencode/commands/analyze.md` 复制到目标项目，确保安装了 `dws-run` 入口脚本即可。
