...
记录每次命令执行情况（命令/资产/耗时/状态等），用于运营统计：多少人用、用哪个命令、失败率、资产热度。

- **默认开启**，静默运行，无 stdout 输出，不影响分析逻辑、无性能影响
- **本地存档**：`~/.analyzer-agent/usage.csv`（完整记录，Excel 可直接打开）
- **内网上报**：每次执行同步 POST 到 `http://10.96.160.123:3000/api/usage`（超时 2s，失败落本地队列下次补发）
- **关闭方式**：设环境变量 `ANALYZER_NO_TELEMETRY=1`，或改 `~/.analyzer-agent/config.json` 的 `telemetry_enabled`
- **数据不上公网**，仅在局域网内收集

**看板**：服务端自带 ECharts 可视化（日活趋势/命令热度/用户活跃/资产 Top10/错误类型/环境分布），部署见 `telemetry-server/`。

**管理本地数据**：
```
python run.py usage               # 查看配置 + 本地统计摘要
python run.py usage --export path # 导出 CSV 副本
...
│   ├── references/          engine/analyzer/view_generator/impact_analyzer/usage
│   ├── templates/           变更清单模板 + 跨资产案例
│   └── SKILL.md             Skill 定义（AI 触发条件）
├── telemetry-server/        运营埋点接收服务端（Node + SQLite + 看板）
...
**Q: 运营埋点会影响分析性能吗？**
不会。埋点在所有交付物落盘之后才执行，CSV 写入 <10ms，HTTP 上报超时 2s（失败静默落队列）。即使埋点完全失败也不影响分析。关闭方式：设环境变量 `ANALYZER_NO_TELEMETRY=1`。

**Q: 运营数据看板怎么部署？**
服务端在 `telemetry-server/`：`npm install && npm start` 启动，监听 3000 端口。浏览器打开 `http://<server-ip>:3000/` 即看板。客户端默认上报到 `http://10.96.160.123:3000`，可用环境变量 `ANALYZER_TELEMETRY_ENDPOINT` 改地址。
